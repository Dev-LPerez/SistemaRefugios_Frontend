  ...
  <div class="flex min-h-screen bg-slate-50 text-slate-900">
    ...
    >
      ...
      <header class="lg:hidden bg-white border-b border-slate-200 h-14 flex items-center px-4 gap-3 sticky top-0 z-20">
        <button @click="toggleSidebar" class="p-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-colors">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
          </svg>
        </button>
        <span class="font-bold text-emerald-700 text-sm font-display uppercase tracking-wider">{{ title }}</span>
      </header>
